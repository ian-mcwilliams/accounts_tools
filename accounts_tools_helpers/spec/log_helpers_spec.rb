...
describe 'LogHelpers' do
  ...
  context 'completion_log' do

    it 'throws an error if an error occurs in the block' do
      expect { LogHelpers.completion_log({ static_text: 'a' }) { raise('err') } }.to raise_error('err')
    end
