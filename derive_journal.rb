...
class DeriveJournal
	...
	def add_exclusions_to_results(accounting_period, results)
		puts "NSPM COUNT: #{results[:errors][:no_successful_pattern_matches].count}"
	...
	def get_exclusions(accounting_period)
		...
		file = FileTools.new(accounting_period, :results, :accounts)
		doc = file.contents
	...
	def start_process(accounting_period)
		file = FileTools.new(accounting_period, :accounts, :accounts)
		doc = file.contents
